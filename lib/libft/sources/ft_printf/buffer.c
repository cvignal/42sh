...
/*   Updated: 2018/12/19 15:48:52 by gchainet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_printf_buffer	*create_buffer(int fd)
{
		...
		const char *s, size_t n)
{
	...
	ft_memcpy(buffer->data + buffer->pos, s, MIN(len, n));
	buffer->pos += MIN(len, n);
	return (0);
}

int			dump_buffer(t_printf_buffer *buffer)
{
	size_t		write_size;
	int			error;
	...
	if (!error)
	{
		write_size = write(buffer->fd, buffer->data, buffer->pos);
		free(buffer->data);
	}
	free(buffer);
