...
/*   Updated: 2019/07/05 14:14:13 by cvignal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_exp_lexer_var_methods(t_exp_lexer *lexer)
{
	...
	while (i <= CHAR_MAX)
	{
		...
		lexer->methods[EXP_STATE_SPECIAL_PARAM]\
			[i] = &exp_lexer_cut_special_param;
		if (ft_isalnum(i) || i == '_')
...
static void	set_exp_lexer_dollar(t_exp_lexer *lexer)
{
	...
	while (i <= CHAR_MAX)
	{
		if (is_special_param(i))
			...
			lexer->methods[EXP_STATE_DOLLAR]\
				[i] = &exp_lexer_curly_expansion_start;
		else
